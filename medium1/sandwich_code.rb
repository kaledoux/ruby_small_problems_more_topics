...
# "Sample Output." You should read the text from a simple text file that
# you provide. (We also supply some example text below, but try the
# program with your text as well.)
#
# This question requires that you read a simple text file.
# Try searching the Web for information on how to do this, and also take a
# look at the File class in the Ruby documentation.
# If you can't figure it out on your own, don't worry:
# just put the data directly into your program; an Array with one element
...
analyzer.process { |file|
  total_lines = 0
  file.each_line('.') {total_lines += 1}
  puts  "Total lines : #{total_lines}"
}
analyzer.process { |file|
  total_words = 0
  file.each_line(' ') {total_words += 1}
  puts "Total words : #{total_words}"
}
analyzer.process { |file|
  total_chars = 0
  file.each_char {|char| total_chars += 1 if /\S/ =~ char}
  puts "Total letters : #{total_chars}"
}
