...
# take an Array and a block, and return the element that contains the
# largest value.
#
...
def max_by(collection)
  ...
  high_value, high_index = nil, nil

  collection.each_with_index do |item, ind|
     if high_value == nil || yield(item) > high_value
       high_value = yield(item)
       high_index = ind
     end
  end
  
  collection[high_index]
end
